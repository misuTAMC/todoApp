...
class EditNote extends StatefulWidget { //EditNote.
  final Note _note;
  const EditNote(this._note, {super.key}); //hàm khởi tạo (constructor) của lớp EditNote
  @override
  State<EditNote> createState() => _EditNoteState();//tạo ra trạng thái (state) của widget EditNote.
}

class _EditNoteState extends State<EditNote> { //lớp con
  TextEditingController? title;// lưu trữ một đối tượng TextEditingController hoặc null.
  TextEditingController? subTitle;

  final FocusNode _focusNode1 = FocusNode();//theo dõi trạng thái focus của trường nhập liệu
  ...
  Widget build(BuildContext context) { //khai báo phương thức build trả về một widget.
    return Scaffold( //Widget chính để xây dựng cấu trúc cơ bản của màn hình.
      ...
      body: SafeArea(
        child: Column( //Sắp xếp các widget theo chiều dọc.
          mainAxisAlignment: MainAxisAlignment.center, // phân bố các widget con dọc theo trục chính
          children: [
      ...
      children: [
        ElevatedButton(
          ...
          onPressed: () { //Hàm xử lý khi người dùng click vào nút bấm
            ...
            Navigator.pop(context);//đóng màn hình chỉnh sửa ghi chú mà không thực hiện cập nhật.
          },
        ...
        ElevatedButton( //
          ...
          onPressed: () { //thao tác người dùng click vào nút bấm
            ...
            child: Padding( //Thêm khoảng đệm cho nội dung bên trong mục
              ...
              child: Container( //Khung chứa chính cho nội dung của mỗi mục
                ...
                child: Column(  //Sắp xếp các widget con theo chiều dọc bên trong container
                  children: [
                    Container( //Khung chứa cho ảnh
                      height: 120,
                      width: 120,
                      decoration: BoxDecoration( //hiển thị ảnh
                        borderRadius: BorderRadius.circular(10),
                        image: DecorationImage(
                          image: AssetImage('assets/taskPicture/$index.png'), //Đường dẫn đến ảnh.
                          fit: BoxFit.cover, //Thiết lập ảnh phủ kín toàn bộ container, có thể bị cắt xén nếu tỷ lệ ảnh không khớp với container.
                        ),
                      ),
                    ),
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: TextField( //Widget chính để người dùng nhập tiêu đề ghi chú
          controller: title, //người dùng nhập vào sẽ được lưu trữ trong biến title
